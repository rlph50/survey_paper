...
g <- ggbr.sim(600,0.22,0.4,0.8,1.0)
# data frame to store the simulated processes
synProcess_22_40_80<-data.frame(obs=1:100)
set.seed(31415925L)

i<-1
while (i<=1000) {
  # from "waveslim" package, use method of Hosking (1984) to generate a realization
  y2<-hosking.sim(600,g)
  synProcess_22_40_80[,paste0("y",i)]<-y2[501:600]
  ss<-spectrum(y2[501:600],plot=F,detrend=F)
  # check the peak in the spectral density is high enough; if not throw this one away.
  if (max(ss$spec[37:43])>7.5) {
    cat(sprintf('Found %d.\n',i))
    i<-i+1
  }
}
saveRDS(synProcess_22_40_80,"synProcess_22_40_80.RDS")
